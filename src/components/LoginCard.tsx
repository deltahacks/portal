...
const LoginButton = (prop: LoginProps) => {
  ...
  const iconStyling = main ? "mr-4 aspect-square w-8" : "mr-2 aspect-square  w-4"
  return (
    <button
  ...
  return (
    <div className=" absolute top-1/2 left-1/2 min-w-[85%] -translate-x-1/2 -translate-y-1/2 md:left-3/4 md:min-w-[40vw] ">
      ...
      <div className="flex flex-col gap-1 rounded-xl border border-zinc-600 bg-zinc-800 p-4 text-white">
        <LoginButton key={"google"} title={"Sign in with Google"} image={google_logo} main={true} />
        ...
        {Object.entries(buttons).map(([key, value]) => (
          value.main ? null : <LoginButton key={key} title={key} image={value.image} main={value.main} />
        ))}
      </div>
    </div>
  );
